...
export class BookService {
  ...
  getPosts() {
    console.log('BookService: Fetching books from http://localhost:3000/api/posts...');
  ...
  updateBook(id: string, title: string, author: string, description: string) {
    ...
    return this.http.put(`http://localhost:3000/api/posts/${id}`, updatedBook)
      .pipe(map(() => {
  ...
  deleteBook(bookId: string) {
    return this.http.delete(`http://localhost:3000/api/posts/${bookId}`)
      .subscribe(() => {
